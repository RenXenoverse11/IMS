...
<style>
  ...
  .sync-button:disabled, .sync-icon-btn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }
</style>
...
<header class="header">
  ...
  <div class="header-actions">
    <button class="icon-button sync-icon-btn" type="button" on:click={handleSyncNow} disabled={syncing} aria-label="Sync latest data">
      <RefreshCw size={17} class={syncing ? 'sync-icon-spin' : ''} />
    </button>

    <button class="icon-button theme-toggle-btn" type="button" on:click={toggleTheme} aria-label="Toggle theme">
      {#if $theme === 'dark'}
        <Sun size={17} color="#FFD600" />
      {:else}
        <Moon size={17} />
      {/if}
    </button>
